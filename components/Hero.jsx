 ...
 */
export default function Hero({ image = null, tone = 'cool', title, strapline, lede, badges = [], route = false, center = false, scrim = null, veil = null, focus = null, tall = false, tornColor = 'var(--paper)' }) {
  return (
    <section className={`hero${tall ? ' hero--tall' : ''}${center ? ' hero--center' : ''}${scrim ? ` hero--scrim-${scrim}` : ''}${veil ? ` hero--veil-${veil}` : ''}${image ? '' : ` hero--plain hero--plain-${tone}`}`}>
